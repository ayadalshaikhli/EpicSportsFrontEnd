    ...
    <div class="account-detail" v-if="user">
      ...
      <label>
        ...
        <span v-if="phoneNumberError" class="error-message"
          >Phone number must be 9 digits.</span
        >
      </label>
      <button
      ...
      <button @click="togglePasswordFields" class="change-password-button">
        Change Password
      </button>
      <div v-if="showPasswordFields" class="password-fields">
        ...
        /></label>
        <span v-if="currentPasswordError" class="error-message">{{
        ...
        /></label>
        <button @click="updatePassword" class="update-password-button">
          Update Password
        </button>
      </div>
...
const fetchUserBookings = async (userId) => {
  try {
    ...
    const response = await axios.get(
      `https://localhost:7063/api/User/${userId}/bookings`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
const updateUserInfo = async () => {
  try {
    if (
      user.value.phoneNumber < 100000000 ||
      user.value.phoneNumber > 999999999
    ) {
    ...
    const response = await axios.put(
      `https://localhost:7063/api/User/${user.value.id}`,
      updatedUser,
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
const updatePassword = async () => {
  try {
    ...
    await axios.put(
      `https://localhost:7063/api/User/${user.value.id}/password`,
      passwordData,
      {
        headers: {
          Authorization: `Bearer ${token}`,
