<template>
  <div class="container">
    ...
    <form @submit.prevent="createCampsite" class="form">
      ...
      <label class="form-label">
        Price Per Night:
        <input v-model.number="campsite.pricePerNight" type="number" step="0.01" min="0" required class="form-input"/>
      </label>
      <label class="form-label">
        Image:
    ...
    <div v-if="campsites.length" class="campsites-list">
      <div v-for="campsite in campsites" :key="campsite.id" class="campsite-card">
        <button class="delete-button" @click="deleteCampsite(campsite.id)">Delete</button>
        <div class="campsite-details">
          ...
          <div v-if="campsite.amenities && campsite.amenities.length" class="amenities">
